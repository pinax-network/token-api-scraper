...
### 1. TRC20 Balances Backfill (`npm run backfill-trc20`)

**Purpose**: Process all historical TRC20 token transfers to populate balance data.
...
**Purpose**: Process all historical accounts to populate native TRX balance data.
...
3. Queries RPC for current native TRX balance
...
- `npm run balances` - TRC20 balances
...
- `npm run backfill-trc20` - TRC20 balances backfill
...
- **TRC20 Balances**: Stores the block number of the transfer that triggered the balance query
...
```
🚀 Starting TRC20 balances BACKFILL service with concurrency: 10
